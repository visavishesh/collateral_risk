...
for config in config_array:
    ...
    data = run_sim(num_simulations=config["num_simulations"],sim_len=config["sim_len"],mu=config["mu"],sigma=config["sigma"],debt_ceiling=config["debt_ceiling"],liquidation_penalty=config["liquidation_penalty"],collateral_cutoff=config["collateral_cutoff"],cdp_distribution=cdp_distribution,jump_probabilities=jump_probabilities,jump_severities=config["jump_severities"]+[0],slippage_function=config["slippage_function"],recovery_time=config["recovery_time"],time_to_start_recovery=config["time_to_start_recovery"],reentry_time=config["reentry_time"],auction_efficiency=config["auction_efficiency"])
    results = aggregate_results(data,config)
    results["num_sim"]=config["num_simulations"]
    results["sim_len"]=config["sim_len"]
    results["mu"]=config["mu"]
    results["sigma"]=config["sigma"]
    results["debt_ceiling"] = config["debt_ceiling"]
    results["liquidation_penalty"]=config["liquidation_penalty"]
    results["collateral_cutoff"]=config["collateral_cutoff"]
    results["jump_probabilities_1"]=jump_probabilities[0]
    results["jump_probabilities_2"]=jump_probabilities[1]
    results["jump_probabilities_3"]=jump_probabilities[2]
    results["jump_severities_1"]=config["jump_severities"][0]
    results["jump_severities_2"]=config["jump_severities"][1]
    results["jump_severities_3"]=config["jump_severities"][2]
    results["recovery_time"]=config["recovery_time"]
    results["time_to_start_recovery"]=config["time_to_start_recovery"]
    results["reentry_time"]=config["reentry_time"]
    results["auction_efficiency"]=config["auction_efficiency"]
