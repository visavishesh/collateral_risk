...
def run_iter(iteration,mu,sigma,collateral_cutoff,liquidation_penalty,sim_len,cdp_distribution,jump_probabilities,jump_severities,slippage_function,recovery_time,time_to_start_recovery,reentry_time,auction_efficiency):    
    ...
    for time_step in range(0,int(t/dt)):
        if time_step==0:
            ...
            slip = slippage(total_debt_purchased,slippage_function)
            
            data[time_step]["slippage_loss"]+=slip*total_debt_purchased
            ...
            for vault in auctioned_vaults:
                dai_obtained += min(vault["collateral_value"]*(1-slip)*auction_efficiency,vault["debt"]*(1+liquidation_penalty))  

            #the amount of loss or gain is equal to the amount of Dai obtained in auction less the debt to recover
            data[time_step]["loss_gain"] += (dai_obtained-total_debt_purchased)
...
def aggregate_results(data,config):
    ...
    avg_losses_nominal=df["loss_gain_total"].mean()

    worst_cutoff=config["worst_case_cutoff"]
    num_simulations=config["num_simulations"]
    sim_len=config["sim_len"]    

    n = max(1,int(num_simulations*sim_len*worst_cutoff))    
    n_worst = data.sort_values("loss_gain",ascending=True).head(n=n)
    
    print(num_simulations*sim_len,n)
    print(worst_cutoff)
    print(n_worst)
    ...
    aggregate["avg_losses_nominal"]=avg_losses_nominal
