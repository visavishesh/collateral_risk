...
def run_iter(iteration,sigma,collateral_cutoff,num_simulations=10,sim_len=365):
...
def run_sim(sigma,collateral_cutoff,num_simulations=10,sim_len=365):
    results=[]

    for simulation in range(num_simulations):
        result = run_iter(simulation,sigma,collateral_cutoff,num_simulations,sim_len)
        results+=result
...
num_simulations=20
sim_len=365
data = run_sim(sigma=.1,collateral_cutoff=1.5,num_simulations=num_simulations,sim_len=sim_len)
total = data.groupby(["iteration"]).sum()
avg = data.groupby(["iteration"]).mean()
debt = avg["debt_supply"]

#print(avg[["sigma","collateral_cutoff"]])
...
#         run_sim(sigma,collateral_cutoff,num_simulations = 10)        
