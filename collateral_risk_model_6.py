...
debt_ceiling = 1.2e6
collateral_cutoff_range = [1.1,1.2,1.3,1.4,1.5,1.6,1.7]
...
for mu in mu_range:
    ...
    for collateral_cutoff in collateral_cutoff_range:
        ...
        data_results+=[{"mu":mu,"collateral_cutoff":collateral_cutoff,"gain-loss":np.average(result_array)}]
        print(mu,collateral_cutoff,liquidation_penalty, np.average(result_array))

fig = px.scatter_3d(pd.DataFrame(data_results), x='mu', y='collateral_cutoff', z='gain-loss',
              color='gain-loss', size_max=18,opacity=0.7)
