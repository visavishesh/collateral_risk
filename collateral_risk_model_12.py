...
#python3 collateral_risk_model_11.py
...
def print_results(data,worst_cutoff):
    #print("data")
    #print(data)
    #print(data.columns)
...
for mu in np.arange(0,.1,.1):
    for sigma in np.arange(1.0,1.4,.1):
        for collateral_cutoff in np.arange(1.1,1.5,.1):
            num_simulations=20
            sim_len=365
            a=0.55
            b=0.8
            ...
            #slippage_function = {"constant":0,"x":-6.68e-10,"x^2":1.18e-16,"x^3":-4.91e-25,"x^4":0,"scalar":1}
            #slippage_function = {"constant":-0.0192,"x":1.4E-9,"x^2":1.37E-16,"x^3":-6.95E-25,"x^4":0,"scalar":1}
            slippage_function = {"constant":3.21E-4,"x":-6.6E-10,"x^2":1.1E-16,"x^3":-4.91E-25,"x^4":0,"scalar":1}
            ...
            while worst_losses<economic_capital:    
                ...
                results = print_results(data,worst_cutoff=.001)
                results["num_sim"]=num_simulations
                results["sim_len"]=sim_len
                results["mu"]=mu
                results["sigma"]=sigma
                results["debt_ceiling"] = debt_ceiling
                results["liquidation_penalty"]=.13
                results["collateral_cutoff"]=collateral_cutoff
                ...
                results["jump_severities_1"]=jump_severities[0]
                results["jump_severities_2"]=jump_severities[1]
                results["jump_severities_3"]=jump_severities[2]
                results["recovery_time"]=recovery_time
                results["time_to_start_recovery"]=time_to_start_recovery
                results["reentry_time"]=reentry_time
                results["auction_efficiency"]=auction_efficiency
